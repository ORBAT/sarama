...
// BUG(ORBAT): QueuingWriterAlt seems to deadlock the producer if more than ChannelBufferSize concurrent Write()s are done. This can be,
// at least temporarily, handled by using a chan with a buffer of ChannelBufferSize-1 as a concurrency limiter.

// QueuingWriterAlt is an io.Writer that writes messages to Kafka. Parallel calls to Write() will cause messages to be queued by the producer.
// Each Write() call will block until a response is received. The number of concurrent writes is limited by ChannelBufferSize
type QueuingWriterAlt struct {
	kp       *Producer
	id       string
	topic    string
...
func (kp *Producer) NewQueuingWriterAlt(topic string) (p *QueuingWriterAlt, err error) {
...
func (c *Client) NewQueuingWriterAlt(topic string, config *ProducerConfig) (p *QueuingWriterAlt, err error) {
	...
	return kp.NewQueuingWriterAlt(topic)
}
...
func (qw *QueuingWriterAlt) Write(p []byte) (n int, err error) {
	n = len(p)
	qw.sem.Acquire()
...
func (qw *QueuingWriterAlt) Closed() (closed bool) {
	select {
...
func (qw *QueuingWriterAlt) CloseBoth() (err error) {
	if qw.Closed() {
...
func (qw *QueuingWriterAlt) Close() error {
	if qw.Closed() {
		return syscall.EINVAL
	}
